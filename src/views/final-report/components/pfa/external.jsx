...
const External = ({stockTakingId}) => {
    ...
    const fetchData = () => {
        setLoading(true)
        api("GET", `final-report/by-location/${stockTakingId}?type=EXTERNAL`).then((res) => {
            setDatas(res.data)
            setLoading(false)
        })
    }
    ...
    return (
        <Card 
            title="Detail Of Pre-Fixed Asset in External Location Qty by Actual"
            style={{
                marginBottom: 20,
            }}
        >
