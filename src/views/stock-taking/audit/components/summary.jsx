...
const Summary = (props) => {
    ...
    const fetchData = () => {
        setLoading(true)
        api("GET", `stock-taking-audit/summary/${data?.id}`).then((res) => {
            setSummary(res.data)
            setLoading(false)
        })
    }
    ...
    const downloadPdfReport = () => {
        setDownloading(true)
        api("GET", `stock-taking-audit/summary/download-pdf/${data?.id}`).then((res) => {
        ...
        <Card className="content-container">
            ...
            <div className="full-width mb-1">
                <Row justify="space-between">
                    ...
                    <Col>
                        { data?.status_audit != 'ACTIVE' ?
                            <p>Data submitted by <b>{data?.audit_submitted_by}</b> on <b>{moment(data?.audit_submitted_at).format('DD MMMM YYYY HH:mm:ss')}</b></p>
                        : null }
                    </Col>
                </Row>
            </div>
