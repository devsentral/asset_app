...
const InternalAsset = () => {
    ...
    const fetchData = () => {
        setLoading(true)
        api("GET", `dashboard/summary-by-asset-status?type=INTERNAL`).then((res) => {
            setSummary(res.data)
            setLoading(false)
        })
    }
    ...
    const handleShowAsset = (category, status) => {
        ...
        api("GET", `dashboard/assets?type=INTERNAL&category=${category}&status=${status}`).then((res) => {
    ...
    const handleChangePagination = (el) => {
        setLoading(true)
        api("GET", `dashboard/assets?type=INTERNAL&category=${category}&status=${status}&page=${el.current}&limit=${el.pageSize}`).then((res) => {
            setDatas(res.data?.data)
            setData(res.data)
            setLoading(false)
        })
    ...
    return (
        <>
            ...
            <Modal title="Status Asset - Internal" open={showModal} 
                footer={null}
                width={1200}
