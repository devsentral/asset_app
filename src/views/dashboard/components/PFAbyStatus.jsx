...
const PFAbyStatus = () => {
    ...
    const fetchData = () => {
        setLoading(true)
        api("GET", `dashboard/summary-by-asset-class?category=PFA`).then((res) => {
            var tempData = []
    ...
    const handleShowAsset = (class_code, category) => {
        ...
        api("GET", `dashboard/assets?type=${category}&category=2&params=class_code&value=${class_code}`).then((res) => {
    ...
    const handleChangePagination = (el) => {
        setLoading(true)
        api("GET", `dashboard/assets?type=${category}&category=2&params=class_code&value=${classCode}&page=${el.current}&limit=${el.pageSize}`).then((res) => {
            setDatas(res.data?.data)
            setData(res.data)
            setLoading(false)
        })
    ...
    return (
        <>
            <Card
                title={`Pre-Fixed Asset - by Asset Class (${summary?.stock_taking?.name ?? ''})`}
                style={{
                    marginBottom: 20,
                }}
            >
